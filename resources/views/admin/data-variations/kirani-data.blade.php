<x-app-layout>
    <title>Bayajidda Global - Kirani Data Services</title>
    ...
    <div class="content">
            ...
            <div class="row align-items-center">
                <div class="col-md-6">
                    <div class="d-flex align-items-center">
                        ...
                        <div>
                            <h3 class="page-title text-primary mb-1 fw-bold">Kirani Data Plans</h3>
                ...
                <div class="col-md-6 text-center mt-2 mt-md-0 d-flex gap-2 justify-content-end align-items-center">
                    <form action="{{ route('admin.kirani-data.sync') }}" method="POST" id="syncForm" class="m-0">
                        @csrf
        ...
        <div class="card border-0 shadow-sm mb-4">
            <div class="card-body">
                <form action="{{ route('admin.kirani-data.index') }}" method="GET" class="row g-3">
                    <div class="col-md-3">
                        <label class="form-label fw-bold">Network</label>
                        <select name="name" class="form-select">
                            <option value="">All Networks</option>
                            @foreach($networks as $network)
                                <option value="{{ $network }}" {{ request('name') == $network ? 'selected' : '' }}>{{ $network }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label fw-bold">Plan ID</label>
                        <input type="text" name="plan_id" class="form-control" placeholder="Search ID..." value="{{ request('plan_id') }}">
                    </div>
                    <div class="col-md-3 d-flex align-items-end gap-2">
                        <button type="submit" class="btn btn-primary flex-fill">
                            <i class="ti ti-search me-1"></i> Filter
                        </button>
                        <a href="{{ route('admin.kirani-data.index') }}" class="btn btn-light flex-fill">
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header bg-white border-bottom py-3">
                <h5 class="card-title mb-0 fw-bold"><i class="ti ti-list-details me-2 text-primary"></i>Available Kirani Plans</h5>
            </div>
                        ...
                        <tbody>
                            @forelse($variations as $variation)
                                <tr class="border-bottom-soft">
                                    ...
                                    <td><code class="text-info bg-soft-info px-2 py-1 rounded">{{ $variation->plan_id }}</code></td>
                                    <td>
                                        <span class="text-muted"><i class="ti ti-calendar-time me-1"></i>30 Days</span>
                                    </td>
                                    <td>
                                        <div class="d-flex flex-column">
                                            <span class="fw-bold text-success fs-15">₦{{ number_format($variation->price, 2) }}</span>
                                    ...
                                    <td class="text-end pe-4">
                                        <div class="d-flex justify-content-end gap-2">
                                            <button class="btn btn-icon btn-sm btn-soft-info rounded-circle edit-plan-btn shadow-sm"
                                                    ...
                                                    data-plan_id="{{ $variation->plan_id }}"
                                                    data-price="{{ $variation->price }}"
                                                    title="Edit Plan">
                                                <i class="ti ti-edit"></i>
                                            </button>
                                            <form action="{{ route('admin.kirani-data.destroy', $variation->id) }}" method="POST" class="delete-plan-form">
                                                @csrf
                                                @method('DELETE')
                                ...
                                <tr>
                                    <td colspan="7" class="text-center py-5">
                                        <div class="py-4">
                                            ...
                                            <p class="text-muted fw-medium">No Kirani plans found. Click "Sync Plans" to fetch from API.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-light">
                    <h5 class="modal-title fw-bold">Add New Kirani Plan</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form action="{{ route('admin.kirani-data.store') }}" method="POST">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-semibold">Plan Name</label>
                                <input type="text" name="name" class="form-control" required placeholder="e.g. MTN 1GB">
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label fw-semibold">Plan ID (External)</label>
                                <input type="text" name="plan_id" class="form-control" required placeholder="e.g. 1000">
                            </div>
                        </div>
                        <div class="row">
    ...
    <div class="modal fade" id="editPlanModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                <div class="modal-header bg-light">
                    <h5 class="modal-title fw-bold">Edit Kirani Plan</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                    ...
                    <div class="modal-body">
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="plan_id" id="edit_plan_id" class="form-control" required>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-12 mb-3">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editButtons.forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    editForm.action = `/admin/kirani-data/${id}/update`;
                    
                    document.getElementById('edit_name').value = this.dataset.name;
                    document.getElementById('edit_plan_id').value = this.dataset.plan_id;
            ...
            if(syncForm) {
                syncForm.addEventListener('submit', function(e) {
                    ...
                    Swal.fire({
                        title: 'Sync Plans?',
                        text: "This will pull the latest Kirani data plans from the API and update your database. Continue?",
                        icon: 'question',
            ...
            deleteButtons.forEach(btn => {
                btn.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        title: 'Delete Plan?',
                        text: "Are you sure you want to delete this Kirani plan? This action cannot be undone.",
                        icon: 'warning',
                        showCancelButton: true,
                        confirmButtonColor: '#d33',
                        cancelButtonColor: '#6c757d',
