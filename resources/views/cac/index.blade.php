<x-app-layout>
    <title>Bayajidda Global - CAC Registration Requests</title>
    <div class="page-body">
        ...
        <style>
            ...
            .financial-card::before { content: ''; position: absolute; top: 0; right: 0; width: 150px; height: 150px; background: rgba(255,255,255,.1); border-radius: 50%; transform: translate(30%,-30%); }
            .financial-card::after  { content: ''; position: absolute; bottom: 0; left: 0; width: 100px; height: 100px; background: rgba(255,255,255,.1); border-radius: 50%; transform: translate(-30%,30%); }
            .stats-label { font-size: .875rem; font-weight: 500; opacity: .9; }
            .stats-value { font-size: 1.5rem; font-weight: 700; letter-spacing: -.025em; }
            @keyframes fadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
            .fade-in-up { animation: fadeIn .5s ease-out forwards; }
            ...
            .form-select:focus, .form-control:focus { border-color: #4e54c8; box-shadow: 0 0 0 .25rem rgba(78,84,200,.1); }
        </style>
            ...
            <div class="col-12">
                <div class="card border-0 shadow-sm overflow-hidden" style="border-radius: 1.25rem;">
                    <div class="card-body p-4 bg-white">
                        <form method="GET" action="{{ route('cac.index') }}">
                                ...
                                <div class="col-lg-6">
                                    <div class="input-group">
                                        ...
                                        <input type="text" name="search" class="form-control border-start-0 bg-light py-2"
                                               placeholder="Search Reference, Agent Name..." value="{{ request('search') }}">
                                    </div>
                                </div>
                                <div class="col-lg-3">
                                    <div class="input-group">
                                ...
                                <div class="col-lg-3">
                                    <div class="d-flex gap-2">
                                        ...
                                        @if(request('status') || request('search'))
                                            <a href="{{ route('cac.index') }}" class="btn btn-outline-danger px-4 py-2 d-flex align-items-center justify-content-center">
        ...
        <div class="card border-0 shadow-sm" style="border-radius: 1.25rem;">
            <div class="card-header bg-white py-4 border-bottom-0 d-flex justify-content-between align-items-center">
                <h5 class="mb-0 fw-bold text-dark d-flex align-items-center">
                    <i class="ti ti-building-community me-2 text-primary fs-22"></i>
                    ...
                    <table class="table table-hover align-middle mb-0">
                        ...
                        <tbody>
                            @forelse($enrollments as $enrollment)
                                @php
                                    $d = is_array($enrollment->modification_data)
                                        ? $enrollment->modification_data
                                        : json_decode($enrollment->modification_data, true);
                                    $d = $d ?? [];
                                    $businessName = $d['business_name_1'] ?? null;
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="d-flex flex-column">
                                                <span class="fw-bold text-dark">{{ $businessName ?? 'N/A' }}</span>
                                                @if($businessType)
                                                    <small class="text-muted">{{ $businessType }}</small>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>
                                        @if($enrollment->reference)
                                            <span class="badge bg-light text-dark border fw-medium px-2 py-1">{{ $enrollment->reference }}</span>
                                        @else
                                    ...
                                    <td class="pe-4 text-end">
                                        <a href="{{ route('cac.show', $enrollment->id) }}"
                                           class="btn btn-icon btn-light btn-sm rounded-circle shadow-sm"
                                ...
                                <tr>
                                    <td colspan="7" class="text-center py-5">
                                        <div class="py-4">
                                            ...
                                            <h6 class="text-muted fw-normal">No CAC registration records found.</h6>
                                            @if(request('search') || request('status'))
                                                <a href="{{ route('cac.index') }}" class="btn btn-link btn-sm mt-2">Clear all filters</a>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
