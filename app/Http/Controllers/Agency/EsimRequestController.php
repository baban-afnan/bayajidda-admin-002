...
{
    /**
     * List eSIM requests with filters and pagination
     */
    public function index(Request $request)
    {
        ...
        $statusCounts = [
            'pending' => AgentService::where('service_type', 'esim')->where('status', 'pending')->count(),
            'processing' => AgentService::where('service_type', 'esim')->where('status', 'processing')->count(),
            'resolved' => AgentService::where('service_type', 'esim')->whereIn('status', ['resolved', 'successful'])->count(),
            'rejected' => AgentService::where('service_type', 'esim')->whereIn('status', ['rejected', 'failed'])->count(),
        ];

        return view('esim-request.index', compact('enrollments', 'search', 'statusFilter', 'statusCounts'));
    }

    /**
     * Show details of a single eSIM request
     */
    public function show($id)
    {
        ...
        return view('esim-request.show', compact('enrollmentInfo', 'statusHistory', 'user'));
    }

    /**
     * Update the status of an eSIM request
     */
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('file')) {
                ...
                $filePath = $file->storeAs('esim-files', $fileName, 'public');
                $baseUrl = rtrim(config('app.url'), '/');
                $fileUrl = $baseUrl . '/storage/esim-files/' . $fileName;
            }
            ...
            DB::commit();
            return redirect()->route('esim.index')->with('successMessage', 'Status updated successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->route('esim.index')->with('errorMessage', 'Failed to update status: ' . $e->getMessage());
        }
    }
    ...
    private function processRefund($enrollment, $forceRefund = false)
    {
        ...
        Transaction::create([
            ...
            'description' => "Refund 80% for rejected eSIM service [{$serviceField->field_name}], Request ID #{$enrollment->id}",
            'type' => 'refund',
            'status' => 'completed',
