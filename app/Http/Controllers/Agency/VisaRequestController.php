...
{
    /**
     * List Visa requests with filters and pagination
     */
    public function index(Request $request)
    {
        ...
        $statusCounts = [
            'pending'    => AgentService::where('service_type', 'visa')->where('status', 'pending')->count(),
            'processing' => AgentService::where('service_type', 'visa')->where('status', 'processing')->count(),
            'resolved'   => AgentService::where('service_type', 'visa')->whereIn('status', ['resolved', 'successful'])->count(),
            'rejected'   => AgentService::where('service_type', 'visa')->whereIn('status', ['rejected', 'failed'])->count(),
        ];

        return view('visa-request.index', compact('enrollments', 'search', 'statusFilter', 'statusCounts'));
    }

    /**
     * Show details of a single Visa request
     */
    public function show($id)
    {
        ...
        return view('visa-request.show', compact('enrollmentInfo', 'statusHistory', 'user'));
    }

    /**
     * Update the status of a Visa request
     */
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('file')) {
                ...
                $filePath = $file->storeAs('visa-files', $fileName, 'public');
                $baseUrl = rtrim(config('app.url'), '/');
                $fileUrl = $baseUrl . '/storage/visa-files/' . $fileName;
            }
            ...
            DB::commit();
            return redirect()->route('visa.index')->with('successMessage', 'Status updated successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->route('visa.index')->with('errorMessage', 'Failed to update status: ' . $e->getMessage());
        }
    }
    ...
    private function processRefund($enrollment, $forceRefund = false)
    {
        ...
        Transaction::create([
            ...
            'description' => "Refund 80% for rejected Visa service [{$serviceField->field_name}], Request ID #{$enrollment->id}",
            'type' => 'refund',
            'status' => 'completed',
