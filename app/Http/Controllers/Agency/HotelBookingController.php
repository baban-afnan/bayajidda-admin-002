...
{
    /**
     * List Hotel bookings with filters and pagination
     */
    public function index(Request $request)
    {
        ...
        $statusCounts = [
            'pending'    => AgentService::where('service_type', 'hotel')->where('status', 'pending')->count(),
            'processing' => AgentService::where('service_type', 'hotel')->where('status', 'processing')->count(),
            'resolved'   => AgentService::where('service_type', 'hotel')->whereIn('status', ['resolved', 'successful'])->count(),
            'rejected'   => AgentService::where('service_type', 'hotel')->whereIn('status', ['rejected', 'failed'])->count(),
        ];

        return view('hotel-booking.index', compact('enrollments', 'search', 'statusFilter', 'statusCounts'));
    }

    /**
     * Show details of a single Hotel booking
     */
    public function show($id)
    {
        ...
        return view('hotel-booking.show', compact('enrollmentInfo', 'statusHistory', 'user'));
    }

    /**
     * Update the status of a Hotel booking
     */
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($request->hasFile('file')) {
                ...
                $filePath = $file->storeAs('hotel-files', $fileName, 'public');
                $baseUrl = rtrim(config('app.url'), '/');
                $fileUrl = $baseUrl . '/storage/hotel-files/' . $fileName;
            }
            ...
            DB::commit();
            return redirect()->route('hotel.index')->with('successMessage', 'Status updated successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect()->route('hotel.index')->with('errorMessage', 'Failed to update status: ' . $e->getMessage());
        }
    }
    ...
    private function processRefund($enrollment, $forceRefund = false)
    {
        ...
        Transaction::create([
            ...
            'description' => "Refund 80% for rejected Hotel service [{$serviceField->field_name}], Request ID #{$enrollment->id}",
            'type' => 'refund',
            'status' => 'completed',
